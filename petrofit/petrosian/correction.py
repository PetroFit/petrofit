...
class PetrosianCorrection:
    ...
    def __init__(self, grid, enforce_range=True):
        ...
        if isinstance(grid, Table):
            ...
            raise TypeError('Input grid should be an astropy Table or file path')
    ...
    def read(cls, grid_file, file_format=None):
        """Read grid from file."""
        grid = Table.read(grid_file, format=file_format)
        return cls(grid)
    ...
    def _validate_input(self, x0, y0, z0):
        assert np.min(self.x) <= x0 <= np.max(self.x), 'P02 is outside of the range of the grid'
        assert np.min(self.y) <= y0 <= np.max(self.y), 'P02 / p03 is outside of the range of the grid'
        assert np.min(self.z) <= z0 <= np.max(self.z), 'P02 / p04 is outside of the range of the grid'
    ...
    def plot_correction(self, p, cmap='hot', target_c='blue', cmap_key='n', colorbar_label=None,
                        suptitle=None, axs=None):
        x0, y0, z0 = self._get_xyz_from_p(p)
